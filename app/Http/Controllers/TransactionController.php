...
class TransactionController extends Controller
{
    ...
    public function getPengeluaranByCategory(Request $request)
    {
        $user = Auth::user();
        ...
        $pengeluaran = DB::table('categories')
            ->leftJoin('transactions', function ($join) use ($user, $tahun, $bulanAngka) {
                $join->on('categories.id', '=', 'transactions.category_id')
                    ->where('transactions.user_id', $user->id)
                    ->where('transactions.type', 'expense')
                    ->whereYear('transactions.transaction_date', $tahun)
                    ->whereMonth('transactions.transaction_date', $bulanAngka);
            })
